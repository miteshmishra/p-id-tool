...
const Editor = () => {
    ...
    const [isDrawingManualConnection, setIsDrawingManualConnection] = useState(false);
    const offsetRef = useRef({ x: 0, y: 0 });

    // P&ID symbols definitions
    const symbols = {
        valve: (ctx: CanvasRenderingContext2D, x: number, y: number, size: number = 40) => {
            ctx.save();
            ctx.beginPath();
        ...
        pump: (ctx: CanvasRenderingContext2D, x: number, y: number, size: number = 40) => {
            ctx.save();
            ctx.beginPath();
        ...
        tank: (ctx: CanvasRenderingContext2D, x: number, y: number, size: number = 40) => {
            ctx.save();
            ctx.beginPath();
            ...
            if (fromNode && toNode) {
                if (connection.path) {
                    ...
                    if (connection.path.length >= 2) {
                        ...
                        const prevPoint = connection.path[connection.path.length - 2];
    ...
    const handleMouseDown = (
        ...
        if (isDrawingManualConnection) {
            setConnectionPoints(prev => [...prev, { x: snappedX, y: snappedY }]);
            return;
        }
    ...
    const handleMouseMove = (
        ...
        setConnections(prevConnections => 
            prevConnections.map(connection => {
                ...
                if (connection.path && (connection.fromNodeId === draggedNode.id || connection.toNodeId === draggedNode.id)) {
    ...
    const handleDoubleClick = (
        ...
        if (isDrawingManualConnection && connectionStart && connectionPoints.length > 0) {
            const { x, y } = getMousePos(e);
            const snappedX = snapToGrid(x);
            const snappedY = snapToGrid(y);
